...
def compute_all_depth_profiles(all_data,statDetails,profile_method,nDepths,mask,stat_mask=None,depthParam='z',radialParam='scale_xy_dist',spec_Drange='MinMax',smooth_factor=0.3,radMax=4):
    '''
...
def compute_diff_profiles(all_data,statDetails,diffDetails,profile_method,nDepths,useSI,mask,depthParam='z',radialParam='x',spec_Drange='MinMax',smooth_factor=0.3,radMax=4):
    '''
...
def plot_avg_depth_profile(p1,avgProfiles,Stats,Colors,ylim,xlim,dx,dy,Ntext,lcolor,fsize,plot_indiv=None,use_decon=True,showSig=False,pthresh=0.05,statCorrType='none'):
    '''
    ...
    for iStat, stat in enumerate(Stats):
        ...
        if showSig:
            if isinstance(statCorrType,Iterable) and not isinstance(statCorrType,str): #custom corrected p-values
                ...
                p1.plot(top[corrected_pvalues <= pthresh] + 0.1,avgProfiles[stat]['norm_depths'][corrected_pvalues <= pthresh],color='k',marker='$*$',linestyle='None')
            elif isinstance(statCorrType,str):
                if statCorrType == 'none':
                    top = avgProfiles[stat]['avg'] + avgProfiles[stat]['stdev']/np.sqrt(np.shape(avgProfiles[stat]['avg'])[0])
                    p1.plot(top[avgProfiles[stat]['p-vals'].pvalue <= pthresh] + 0.1,avgProfiles[stat]['norm_depths'][avgProfiles[stat]['p-vals'].pvalue <= pthresh],color='k',marker='$*$',linestyle='None')
                else:
                    top = avgProfiles[stat]['avg'] + avgProfiles[stat]['stdev']/np.sqrt(np.shape(avgProfiles[stat]['avg'])[0])
        ...
        p1.text(Ntext[0], Ntext[1], 'n=%d hemis' %avgProfiles[stat]['Nsamp'], color=lcolor, fontsize=fsize*.5, fontstyle='italic')
        ...
        if isinstance(plot_indiv,dict):
            ...
            if use_decon:
                ...
                p1.plot(plot_indiv[stat]['avg'], np.tile(avgProfiles[stat]['norm_depths'],[avgProfiles[stat]['Nsamp'],1]), color=Colors[iStat],alpha=0.3)
    ...
    for iDiff, diff in enumerate(Diffs):
        ...
        if showSig:
            if isinstance(statCorrType,Iterable) and not isinstance(statCorrType,str): #custom corrected p-values
                ...
                corrected_pvalues = statCorrType[diff]
                p2.plot(top[corrected_pvalues <= pthresh] + 0.1,avgDiffs[diff]['norm_depths'][corrected_pvalues <= pthresh],color='k',marker='$*$',linestyle='None')
            elif isinstance(statCorrType,str):
                if statCorrType == 'none':
                    top = avgDiffs[diff]['avg'] + avgDiffs[diff]['stdev']/np.sqrt(np.shape(avgDiffs[diff]['avg'])[0])
...
def get_deep_layer_dist(df, depth_var, deep_pct):
    ...
    deep_mean = np.mean(get_lmnv(df[deep]))
    deep_std = np.std(get_lmnv(df[deep]))
...
def plot_mnv_histograms(lmnv, deep_lmnv, mnv_mask, deep_pct, key, k_i, NROIs, fsize, pad=0.1, figsize=(6.5,6.5)):
    """
    Plots histograms for a given dataframe.
    """
    fthresh = plt.figure("thresh",figsize=figsize)
...
def plot_depth_voxel_loss(z, mnv_mask, nDepths, NROIs, key, k_i, fsize, pad=0.1, figsize=(15,3)):
    """
    ...
    fdepth_hist = plt.figure("depth",figsize=figsize)
    
    plt.subplot(2,NROIs,(k_i+1))
    plt.hist(z,bins=np.linspace(np.min(z),nD,nDepths+1))
    plt.title(key+"\n Depth Hist",fontsize=fsize)
    plt.xticks(ticks = None, labels = None, fontsize=0.5*fsize)
    plt.xlim([0,1])
    plt.ylim([0,1.3*np.max(np.bincount(np.digitize(z,np.arange(np.min(z),nD,nD/nDepths))))])
    plt.yticks(ticks = None, labels = None, fontsize=0.5*fsize)
    plt.hist(z[mnv_mask],bins=np.linspace(np.min(z),nD,nDepths+1),alpha=0.7)
    plt.legend(["unmasked","masked"],fontsize=0.7*fsize)
    
    plt.subplot(2,NROIs,(k_i+1)+NROIs)
    plt.bar(np.linspace(np.min(z)+nD/(2*nDepths),nD-nD/(2*nDepths),nDepths),frac_included,width=nD/nDepths,color='tomato')
    plt.xticks(ticks = None, labels = None, fontsize=0.5*fsize)
    plt.yticks(ticks = None, labels = None, fontsize=0.5*fsize)
...
def compare_conditions(profiles,conditions):
    '''
    Compare Conditions
...
def compute_avg_rad_profile(rois,statDetails,diffDetails,plotStats,plotDiffs,prop_err,useSI):
    '''
