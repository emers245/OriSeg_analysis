...
#%% Show patches
#From Cheryl's code, this just fits the ellipse by using the eigenvectors of 
#the covariance matrix as the major and minor axes whose lengths are determined
#by the eigenvalues.
...
for iR, label in enumerate(all_data.keys()):
    ...
    ax.set_xlim([minx, minx+20])
    ax.set_ylim([miny, miny+20])
    ax.add_patch(ellipse)
...
if savefigs:
    fig.savefig(os.path.join(figDir,'ellipses.%s' %(fig_format)))
...
plot_flat_patches(all_data, cond, pthresh, cmax, red, fcolor, fontsize, lcolor, xvar = 'x', yvar = 'y', pval_var = 'task p-val')

if savefigs:
    fig.savefig(os.path.join(figDir,"%s_xy.%s" %(cond,fig_format)))
...
plot_smooth_patches(all_data, cond, pthresh, cmax, smooth_kernel2D, smooth_factor, resamp_factor, xlims=None, ylims=None, xvar='u', yvar='v')
    
if savefigs:
    fig2.savefig(os.path.join(figDir,"%s_xy_smoothed.%s" %(cond,fig_format)))
