...
mainDir = '.'
figDir = os.path.join(mainDir,'figs')
fig_format = 'svg'
statCorrType = 'fdr_bh'
...
#or exclude
exclude_initial = ['pnr143_V1_tgt_rh','pnr161_V1_tgt_lh','pnr161_V1_tgt_rh','pnr256_V1_tgt_lh','pnr256_V1_tgt_rh','pnr352_V1_tgt_lh','pnr352_V1_tgt_rh','pnr579_V1_tgt_lh','pnr668_V1_tgt_rh']
...
for r_i, ring in enumerate(rings):
    ...
    for iR, label in enumerate(all_data.keys()):
        ...
        pcm = ax.scatter(df['x'],df['y'],c=df['ctr-sur_unwarp'],s=0.2,cmap=cmap,vmin=-1,vmax=1)
    ...
    if savefigs:
        frad.savefig(os.path.join(figDir,'rings','rings_map_%s.%s' %(ring,fig_format)))
...
fix_axes(p2, lcolor=lcolor, fcolor=fcolor)
xlim = [-0.5,1.5]
plot_avg_diff_profile(p2,ctrTaskDiffs,['iso-sur'],['black'],ylim,xlim,dx,dy,Ntext,lcolor,fsize,useSI,showSig=True,pthresh=pthresh,statCorrType=pvals_task_ctr)#,plot_indiv=diffProfiles['ctr'])
plt.title('ctr')
...
fix_axes(p2, lcolor=lcolor, fcolor=fcolor)
xlim = [-0.5,1.5]
plot_avg_diff_profile(p2,borTaskDiffs,['iso-sur'],['black'],ylim,xlim,dx,dy,Ntext,lcolor,fsize,useSI,showSig=True,pthresh=pthresh,statCorrType=pvals_task_border)#,plot_indiv=diffProfiles['border'])
plt.title('bor')
...
fix_axes(p2, lcolor=lcolor, fcolor=fcolor)
xlim = [-0.5,1.5]
plot_avg_diff_profile(p2,surTaskDiffs,['iso-sur'],['black'],ylim,xlim,dx,dy,Ntext,lcolor,fsize,useSI,showSig=True,pthresh=pthresh,statCorrType=pvals_task_sur)#,plot_indiv=diffProfiles['sur'])
plt.title('sur')
