...
Update (02/21/2024): This version analyzes only data from a single hemisphere
per participant.
"""
#import nibabel
import os, glob
...
datasets_all = glob.glob(os.path.join(mainDir, 'roi_data_manualSeg/target_roi_manual', 'pnr???_??_???_??.csv'))
...
for e_i, excl in enumerate(exclude_initial):
    datasets_all.remove(os.path.join(mainDir,'roi_data_manualSeg/target_roi_manual',excl+'.csv'))
    #datasets.remove(os.path.join(mainDir,'roi_data_manualSeg/target_filled',excl+'.csv'))        

# Randomly choose one hemisphere per subject
roi_selection = 'random' #'random'
if roi_selection == 'random':
    np.random.shuffle(datasets_all)
    datasets = []
    subjIDs = []
    for dset in datasets_all:
        f = os.path.split(dset)[1]
    ...
    for dset in datasets_all:
        f = os.path.split(dset)[1]
        sID = f[f.find('pnr'):f.find('pnr')+16]
        if sID in include:
            subjIDs.append(sID)
            datasets.append(dset)
